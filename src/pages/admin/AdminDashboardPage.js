...
export default function AdminDashboardPage() {
  const { markets, allProducts } = useMarket();

  // Simulated & calculated stats
  const totalProducts = allProducts.length;
  const totalMarkets = markets.length;
  
  // Orders mock from localStorage or defaults
  const orders = JSON.parse(localStorage.getItem('7shop_admin_orders_v1') || '[]');
  const totalOrders = orders.length || 18;
  const totalRevenue = orders.reduce((sum, o) => sum + (o.total || 0), 0) || 1485.50;

  // Stock alerts (stock <= 20)
  ...
  return (
    <div className="admin-dashboard-root">
      ...
      <div className="stats-grid-row">
        ...
        <div className="kpi-card">
          <div className="kpi-info-col">
            <span className="kpi-label">Commandes Totales</span>
            <span className="kpi-value">{totalOrders}</span>
            <span className="kpi-subtext">↑ +14% cette semaine</span>
          </div>
        ...
        <div className="kpi-card">
          <div className="kpi-info-col">
            <span className="kpi-label">Marchés Actifs</span>
            <span className="kpi-value">{totalMarkets}</span>
            <span className="kpi-subtext">Multi-boutiques opérationnelles</span>
          </div>
        ...
        <div className="kpi-card">
          <div className="kpi-info-col">
            <span className="kpi-label">Articles au Catalogue</span>
            ...
            <span className="kpi-subtext" style={{ color: lowStockProducts.length > 0 ? '#f59e0b' : '#10b981' }}>
              {lowStockProducts.length} alertes stock
            </span>
          </div>
      ...
      <div className="dashboard-sections-grid">
        ...
        <div className="admin-card-panel">
          <div className="admin-card-header">
            <h3><span>🏬</span> Performance & Répartition par Marché</h3>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: '20px', fontSize: '0.82rem', color: '#64748b' }}>
                    ...
                    <span>📂 <strong>{categoryCount}</strong> catégories</span>
        ...
        <div className="admin-card-panel">
          ...
          <div className="admin-card-header" style={{ marginTop: '20px' }}>
            <h3><span>⚠️</span> Alertes Stock Faible</h3>
          </div>
          ...
          {lowStockProducts.length > 0 ? (
            ...
            <p style={{ fontSize: '0.82rem', color: '#10b981', fontWeight: '600' }}>✓ Tous les stocks sont suffisants.</p>
          )}
        </div>
      </div>
    </div>
